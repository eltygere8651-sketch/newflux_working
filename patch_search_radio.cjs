...
// 1. Remove the Promise.all logic and restore upnext for clicking a search result
...
// 2. Improve the infinite radio logic in handleNext to trigger when <= 3 and append!
const nextRadioTarget = `      // Infinite radio logic: if queue is empty and we are playing a youtube track, fetch more related tracks
...
const nextRadioReplacement = `      // Infinite radio logic: if queue is running out, fetch more related tracks and append
...
console.log('Fixed infinite radio');
