...
const targetLogic = `            if (snapshot.exists()) {
...
const replaceLogic = `            let tStart = null;
            ...
            if (deviceHasTrial && u.email !== "eltygere8651@gmail.com") {
              const shouldApplyDeviceTrial = isVipAccount || (!isSubExpired && !isTrialExpired && !isPlanNone);
...
if (code.includes(targetLogic)) {
  ...
  console.log('Fixed FirebaseProvider logic');
} else {
